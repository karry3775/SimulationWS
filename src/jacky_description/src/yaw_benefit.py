...
# Global variables with yaw correction
X_gt = []
Y_gt = []
YAW_gt = []
X_vo = []
...
# Global variables without yaw correction
nX_vo = []
nY_vo = []
nYAW_vo = []
...
if __name__ == "__main__":
    ...
    try:
        vo_sub_w_yaw = rospy.Subscriber("/stereo_odometer_with_yaw_correction/pose_with_yaw_correction", PoseStamped, stereo_cb_w_yaw)
        vo_sub_wo_yaw = rospy.Subscriber("/stereo_odometer_without_yaw_correction/pose_without_yaw_correction", PoseStamped, stereo_cb_wo_yaw)
        rospy.spin()
        ...
        axs[0,0].plot(X_vo, Y_vo,'b-', label = "VO with yaw")
        axs[0,0].plot(nX_vo, nY_vo, 'm-', label = "VO without yaw")
        axs[0,0].set_title("Robot Path")

        axs[0,1].plot(YAW_gt,'r-', label = "GT")
        axs[0,1].plot(YAW_vo,'b-', label = "VO with yaw")
        axs[0,1].plot(nYAW_vo, 'm-', label = "VO without yaw")
        axs[0,1].set_title("Yaw")


        axs[1,0].plot(DELTA_X_GT, 'r-', label = "GT")
        axs[1,0].plot(DELTA_X_VO, 'b-', label = "VO with yaw")
        axs[1,0].plot(nDELTA_X_VO, 'm-', label = "VO without yaw")
        axs[1,0].set_title("Delta x")

        axs[1,1].plot(DELTA_Y_GT, 'r-', label = "GT")
        axs[1,1].plot(DELTA_Y_VO, 'b-', label = "VO with yaw")
        axs[1,1].plot(nDELTA_Y_VO, 'm-', label = "VO without yaw")
